...
##############################
# App 接続テスト用サーバーの作成（プライベートサブネット）
##############################
resource "aws_instance" "ec2_test_instance_in_web_subnet" {
  ami                    = data.aws_ami.amazon_linux.id
  instance_type          = "t2.micro"
  ...
  tags = {
    Name = "for-app-test-in-web-subnet"
  }
}

##############################
# DB 接続テスト用サーバーの作成（プライベートサブネット）
##############################
resource "aws_instance" "ec2_test_instance_in_app_subnet" {
  ami                    = data.aws_ami.amazon_linux.id
  instance_type          = "t2.micro"
  subnet_id              = aws_subnet.private_subnet_for_app.id
  vpc_security_group_ids = [aws_security_group.sg_for_app.id]
  key_name               = aws_key_pair.basic_app_ecr_test.key_name
  iam_instance_profile   = aws_iam_instance_profile.ec2_instance_profile.name
  ...
  tags = {
    Name = "for-db-test-in-app-subnet"
  }
}
